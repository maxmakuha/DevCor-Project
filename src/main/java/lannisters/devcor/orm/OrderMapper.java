...
public class OrderMapper implements RowMapper<Order> {
	...
	public Order mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if(rs.getObject("author_id") == null){
			order.setAuthorId(-1);
			order.setAuthorEmail("Deleted");
			order.setAuthorName("Deleted");
			order.setAuthorSurname("");
		}else{
		...
		if(rs.getObject("technician_id") == null){
			order.setTechnicianId(-1);
			order.setTechnicianEmail("Deleted");
			order.setTechnicianName("Deleted");
			order.setTechnicianSurname("");
		}else{
